...
int obstacle_presence = 0;
int obstacle_presence_thresh = 3;
...
{
  get_messages_from_serial();
  if (sonarCounter == 100 || obstacle) {
   
   sonarCounter = 0;
   if(measureDistance.check() == 1){
     actualDistance = MeasureDistance();
     if (actualDistance > 5 && actualDistance < 40){
       obstacle_presence = min(obstacle_presence + 1, 2*obstacle_presence_thresh);
       //analogWrite(SPEED_LEFT, convert_to_pwm(float(0)));
       //analogWrite(SPEED_RIGHT, convert_to_pwm(float(0)));
       delay(50);
     }else {
       obstacle_presence = max(obstacle_presence-1, 0);
     }

     if (obstacle_presence > obstacle_presence_thresh){
       obstacle=true;
       Serial.write(1);
       delay(400);
     }else{
